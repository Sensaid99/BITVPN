...
def escape_html(text: str) -> str:
    """Escape HTML special characters for safe display in Telegram (parse_mode='HTML')."""
    if not text:
        return ""
...
def _plan_type_to_duration_key(plan_type: str) -> str:
    """6_months_3 -> 6_months (для длительности)."""
...
def calculate_end_date(plan_type: str) -> datetime:
    ...
    key = _plan_type_to_duration_key(plan_type)
...
def get_plan_duration_key(plan_type: str) -> str:
    """Return base plan key for duration/name (6_months_3 -> 6_months)."""
...
def get_plan_emoji(plan_type: str) -> str:
    ...
    return emojis.get(get_plan_duration_key(plan_type), '📦')
