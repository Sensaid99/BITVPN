...
Счётчик берётся из Happ API `list-install`. Если показывается 0 или «Нет устройств» при том что ссылка с `installid` есть:

1. **На сервере в `.env` должно быть именно:**  
   `HAPP_API_URL=https://api.happ-proxy.com`  
   (не `https://happ-proxy.com` — там 404).

2. **Перезапустите API** после смены:  
   `sudo systemctl restart miniapp-api`

3. **Откройте мини-апп → «Устройства»** и на сервере посмотрите логи:
   ```bash
...
4. **Проверить вручную с сервера** (подставьте свой `auth_key` и при необходимости `provider_code`):
   ```bash
