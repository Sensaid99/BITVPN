...
1. **Проверить с ПК в браузере** (подставьте свои `provider_code` и `auth_key` из `.env`):
   ```
   ...
   - Если в браузере тоже **404 или HTML** — значит, по этому адресу API уже не отдаётся. Нужен актуальный URL от Happ.

2. **Уточнить у поддержки Happ** (например, в Telegram или на сайте):
...
4. После смены **HAPP_API_URL** в `.env` на сервере обязательно перезапустить API:
   ```bash
   sudo systemctl restart miniapp-api
   ```
