        ...
        try:
            ...
            if not subscription_link and sub.end_date and sub.end_date > datetime.utcnow():
                try:
                    ...
                    if not subscription_link:
                        base_url = (_os_environ_get("HAPP_SUBSCRIPTION_URL") or "").strip()
                        if base_url:
                            subscription_link = base_url.rstrip("/")
            ...
            try:
                if subscription_link and str(subscription_link).strip().lower().startswith("http"):
                    if (_os_getenv("HAPP_ENCRYPT_SUBSCRIPTION_LINKS") or "").strip().lower() in ("1", "true", "yes"):
...
async def miniapp_create_payment(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Ошибка при создании платежа. Укажите DATABASE_URL и BOT_TOKEN в настройках или запустите API на своём сервере; логи: journalctl -u miniapp-api -f (см. docs/deploy/КАК_НАЙТИ_ЛОГИ_НА_СЕРВЕРЕ.txt)."
        )
